...
// Desplegar: supabase functions deploy jira-materiales-webhook --no-verify-jwt
...
/** Normaliza el nombre de estado de Jira a un valor canónico para la vista de stock.
 *  Estados en Jira: "En Cacheuta", "En deposito", "Entregado", "SOLICITADO".
 *  Solo status = 'en deposito' suma en stock; 'En Cacheuta' no cuenta. */
...
async function fetchIssueFromJira(issueKey: string): Promise<{
  ...
  const res = await fetch(
    `${baseUrl}/rest/api/3/issue/${issueKey}?fields=issuetype,status,${CUSTOM_FIELD_MATERIAL_CODE},${CUSTOM_FIELD_QUANTITY},created,updated`,
    {
      headers: {
  ...
  const updated =
    typeof (fields as { updated?: string }).updated === "string"
      ...
      : null;
  return { material_code, quantity, status, issuetype, created, updated };
}
